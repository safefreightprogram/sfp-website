...
function formatArticlesForNewsletter(articles) {
  return articles.map(article => {
    return `### ${article.title}\n${article.summary || article.body || 'Content summary not available'}`;
  }).join('\n\n');
}

function generateEnhancedFallbackContent(segment) {
...
Important safety updates for winter driving conditions including new road rules and equipment requirements. Stay informed about changes affecting delivery routes and operational safety procedures.`;
  ...
  return {
    ...
    issueId: issueId,
    sources: [] // No sources for fallback content
  };
}

function extractArticleFromSearchResult(result, source) {
  return {
    title: result.title || 'Industry Update',
    summary: result.snippet || result.description || 'Details available at source link.',
