...
const CONFIG = {
  ...
  ANALYTICS: {
    TRACKING_PIXEL_BASE: 'YOUR_DEPLOYED_URL'?action=track',
...
function generateNewsletterContent(segment, sources) {
  // First try to get real current content
  const currentContent = fetchCurrentIndustryContent(segment);
  
  if (currentContent && currentContent.articles.length >= 3) {
    return {
      content: formatArticlesForNewsletter(currentContent.articles),
      issueId: 'ISSUE-' + new Date().toISOString().substring(0, 10) + '-' + segment,
      sources: currentContent.articles
    };
  }
  
  // Enhanced fallback with specific URLs
  return generateEnhancedFallbackContent(segment);
}

function fetchCurrentIndustryContent(segment) {
  try {
    const searches = segment === 'pro' ? [
      { query: 'site:nhvr.gov.au prosecution 2025', source: 'NHVR' },
      { query: 'site:austlii.edu.au "heavy vehicle" OR "chain of responsibility" 2025', source: 'AustLII' },
      { query: 'site:safework.nsw.gov.au transport safety alert 2025', source: 'WorkSafe NSW' },
      { query: 'site:accc.gov.au transport logistics 2025', source: 'ACCC' }
    ] : [
      { query: 'site:nhvr.gov.au driver fatigue management 2025', source: 'NHVR' },
      { query: 'site:powertorque.com.au licence medical 2025', source: 'PowerTorque' },
      { query: 'site:twu.com.au safety alert 2025', source: 'TWU' },
      { query: 'site:healthyheads.org.au driver wellbeing 2025', source: 'Healthy Heads' }
    ];
    ...
    for (const search of searches) {
      try {
        const results = performWebSearch(search.query);
        if (results && results.length > 0) {
          const article = extractArticleFromSearchResult(results[0], search.source);
          if (article) articles.push(article);
        }
      } catch (e) {
        console.log(`Search failed for ${search.source}: ${e.message}`);
      }
    }
...
function performWebSearch(query) {
  // Note: Google Apps Script doesn't have direct search API access
  // This is a placeholder for RSS/web scraping implementation
  const rssSources = {
    'site:nhvr.gov.au': 'https://www.nhvr.gov.au/rss.xml',
    'site:austlii.edu.au': null, // No RSS - would need scraping
    'site:safework.nsw.gov.au': null,
    'site:accc.gov.au': null
  };
  
  // Implementation would use UrlFetchApp to check RSS feeds
  // Return empty array for now - this needs manual RSS parsing
  return [];
}

function searchAustLIICases() {
  try {
    // AustLII doesn't have RSS, so we need to scrape recent decisions
    const searchUrls = [
      'https://www.austlii.edu.au/cgi-bin/sinodisp/au/cases/cth/FCA/',
      'https://www.austlii.edu.au/cgi-bin/sinodisp/au/cases/nsw/NSWSC/',
      'https://www.austlii.edu.au/cgi-bin/sinodisp/au/cases/vic/VSC/'
    ];
    ...
    for (const url of searchUrls) {
      try {
        ...
        if (response.getResponseCode() === 200) {
          ...
          const recentCases = extractTransportCasesFromHTML(html);
          cases.push(...recentCases);
        }
      } catch (e) {
    ...
    return cases.filter(c => isTransportRelated(c.title)).slice(0, 2);
  } catch (error) {
    console.error('AustLII search failed:', error);
    return [];
  }
}

function extractTransportCasesFromHTML(html) {
  // Basic HTML parsing for case listings
  const casePattern = /<a href="([^"]+)"[^>]*>([^<]+)<\/a>/gi;
  const cases = [];
  let match;
  
  while ((match = casePattern.exec(html)) !== null) {
    const [, href, title] = match;
    if (isTransportRelated(title)) {
      cases.push({
        title: title.trim(),
        url: href.startsWith('http') ? href : `https://www.austlii.edu.au${href}`,
...
function isTransportRelated(title) {
  const keywords = [
    'heavy vehicle', 'chain of responsibility', 'transport', 'logistics',
    'freight', 'trucking', 'dangerous goods', 'fatigue', 'mass and dimension',
    'HVNL', 'NHVR', 'driver licence', 'vehicle standards'
  ];
...
// Use actual source URL if available
    let url = (sources && sources[index] && sources[index].url) ? 
      sources[index].url : 
      generateSpecificURL(title, body);
    let category = 'Update';
...
function generateSpecificURL(title, body) {
  const titleLower = title.toLowerCase();
  const bodyLower = body.toLowerCase();
  
  // Map to most likely specific pages rather than generic ones
  if (titleLower.includes('prosecution') || titleLower.includes('enforcement')) {
    return 'https://www.nhvr.gov.au/news/prosecutions-and-court-outcomes';
  }
  if (titleLower.includes('consultation') || titleLower.includes('public comment')) {
    return 'https://www.nhvr.gov.au/have-your-say';
  }
  if (titleLower.includes('worksafe') || bodyLower.includes('workplace safety')) {
    return 'https://www.safework.nsw.gov.au/news';
  }
  if (titleLower.includes('austlii') || titleLower.includes('court') || titleLower.includes('tribunal')) {
    return 'https://www.austlii.edu.au/au/cases/';
  }
  
  // Default to news sections rather than home pages
  return 'https://www.nhvr.gov.au/news';
}
