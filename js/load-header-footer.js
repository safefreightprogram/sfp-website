...
const res = await fetch(url + bust, { cache: "no-store" });
  ...
  function initDropdowns(rootEl) {
    ...
    // Clicks inside a panel don't bubble to the outside-click handler
    // so the user can type in the search box or read the menu without it closing
    IDS.forEach(function (id) {
      var p = panel(id);
      if (p) {
    ...
    // Clicking anywhere outside closes all panels (bubble phase)
